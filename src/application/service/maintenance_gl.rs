//! Outbound GL-posting port (hand-authored, user-owned) — maintenance's side of the GL-posting contract.
//!
//! Maintenance emits ONE balanced posting per completed visit, `source_type = "maintenance"`,
//! `posting_type='original'`, `source_id = visit_id`:
//!   Dr Maintenance Expense (total) · Cr Inventory Parts (parts value) · Cr Labor Payable (labor).
//! Because `total = parts + labor`, it balances. The 9th GL producer. Reached only through a `GlPostSink`;
//! the ACL adapter maps the envelope into accounting's `PostingRequest`. ZERO normal Cargo edge to
//! accounting — the envelope is the wire contract, duplicated per producer by design.
