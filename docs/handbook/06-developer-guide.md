...
metaphor migration generate Invoice maintenance
metaphor migration run
...
(`maintenance` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed maintenance          # run Rust seeders
metaphor migration generate-seeds maintenance  # emit SQL seed files
```
